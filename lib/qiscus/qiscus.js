...
var qiscus = (function(_) {
	...
	User.prototype.loadRooms = function() {
		...
		this.agent.listRooms(
			...
			function() {
				...
				// For each unread Rooms, find its Topics that contains
				// unread Comments, and load them all.
				_.each(unreadRooms, function(unreadRoom) {
					_this.loadRoom(unreadRoom.id)
					.then(function() {
						var unreadTopics = _.filter(unreadRoom.topics, function(topic) {
	...
	User.prototype.receiveComment = function(topicId, comment) {
		...
		if (comment.isSent && comment.isAttachment() && comment.isAttachmentImage()) {
