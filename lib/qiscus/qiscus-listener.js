...
qiscusListener.handleParticipantJoined = function(participantJoinedData, user, roomId) {
	// For now, simply reload the Room to get the new Participant data.
...
qiscusListener.handleCommentDeleted = function(commentDeletedData, user) {
	var topicId = commentDeletedData.topic_id;
	var room    = user.findRoomOfTopic(topicId);

	if (room) {
		var commentId = commentDeletedData.comment_id;
		var topic     = room.getTopic(topicId);
